...
return tokenSComment(stream,state);}else{stream.eatWhile(/[\a-zA-Z0-9\-_.]/);if(stream.peek()==")"||stream.peek()=="/")return ret("string","string");return ret("number","unit");}}
...
else{stream.eatWhile(/[\w\\\-_.%]/);if(stream.eat("(")){return ret(null,ch);}else if(stream.current().match(/\-\d|\-.\d/)){return ret("number","unit");}else if(inTagsArray(stream.current())){return ret("tag","tag");}else if((stream.peek()==")"||stream.peek()=="/")&&stream.current().indexOf('.')!==-1){return ret("string","string");}else{return ret("variable","variable");}}}
