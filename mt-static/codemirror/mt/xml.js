...
function inAttribute(quote){return function(stream,state){while(!stream.eol()){if(stream.match(/^<\$?(MT:?)((?:<[^>]+?>|"(?:<[^>]+?>|.)*?"|'(?:<[^>]+?>|.)*?'|.)+?)([-]?)[\$\/]?>|^<\/MT[^>]+>/i))
continue;else if(stream.next()==quote){state.tokenize=inTag;break;}}
return"string";};}
