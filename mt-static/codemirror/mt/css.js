...
return ret("comment","comment");}
function tokenString(quote){return function(stream,state){var escaped=false,ch;while(!stream.eol()){if(stream.match(/^<\$?(MT:?)((?:<[^>]+?>|"(?:<[^>]+?>|.)*?"|'(?:<[^>]+?>|.)*?'|.)+?)([-]?)[\$\/]?>|^<\/MT[^>]+>/i))
continue;ch=stream.next();if(ch==quote&&!escaped)
