...
{evt=evt||event;var element=evt.target||evt.srcElement;this.init();this.pickFunc=pickFunc;this.window.show();var pos=TC.getAbsolutePosition(element);this.window.element.style.left=pos.left+"px";this.window.element.style.top=pos.top+"px";return false;}
TC.ColorPicker.prototype.pickColor=function(color)
{this.window.hide();this.addRecentColor(color);this.showRecentColors();this.bakeRecentColors();if(this.pickFunc)
this.pickFunc(color);}
TC.ColorPicker.prototype.addRecentColor=function(color)
{if(!color)
{if(!this.recentNone)
return;color="";}
var colors=[color];for(var i in this.recentColors)
if(color!=this.recentColors[i])
colors[colors.length]=this.recentColors[i];if(colors.length>this.recentElements.length)
colors.length=this.recentElements.length;this.recentColors=colors;}
...
{if(!this.cookie)
return;if(!this.cookie.fetch())
return false;if(!this.cookie.value)
return false;var colors=this.cookie.value.split(",");for(var i in this.recentColors)
this.recentColors[i]=colors[i]?colors[i]:"";}
TC.ColorPicker.prototype.bakeRecentColors=function()
{if(!this.cookie)
return;var colors=[];this.cookie.value=this.recentColors.join(",");this.cookie.expires=new Date();this.cookie.expires.setFullYear(parseInt(this.cookie.expires.getFullYear())+100);this.cookie.bake();}
TC.ColorPicker.prototype.setElementColor=function(element,color,title)
{if(!element)
return;if(!color)
color=this.allowNone?"":this.defaultColor;if(!title)
title=color;if(!title)
title="";if(!color)
element.className=this.noneClassName;else if(element.className)
element.className="";element.style.backgroundColor=color;element.setAttribute("color",color);element.setAttribute("title",title);}
TC.ColorPicker.prototype.clickColor=function(evt)
{evt=evt||event;var element=evt.target||evt.srcElement;var color=element?element.getAttribute("color"):"";this.pickColor(color);return TC.stopEvent(evt);}
TC.ColorPicker.prototype.init=function()
{if(this.inited)
return;this.recentColors=[];this.recentElements=[];this.window=new TC.Window();this.window.quick=true;this.window.element.className=this.className;this.window.setStyle(this.style);if(TC.Cookie)
this.cookie=new TC.Cookie(this.cookieName);var self=this;this.clickColorClosure=function(evt){return self.clickColor(evt);};var recent=this.createPicker(this.recentClassName,this.recentCount);var picker=this.createPicker(this.colorsClassName,this.colors.length,this.colors);this.fetchRecentColors();this.showRecentColors();this.window.element.appendChild(recent);this.window.element.appendChild(picker);this.window.attach();this.inited=true;}
TC.ColorPicker.prototype.createPicker=function(className,count,colors)
{if(!colors)
colors=[];var div=document.createElement("div");div.className=className;var swatch;for(var c=0;c<count;c++)
{if(swatch)
swatch=swatch.cloneNode(true);if(!swatch)
{swatch=document.createElement("a");swatch.appendChild(document.createTextNode(" "));}
swatch.onclick=this.clickColorClosure;var color="";if(c<colors.length)
color=colors[c];else
{this.recentColors[this.recentColors.length]=color;this.recentElements[this.recentElements.length]=swatch;}
this.setElementColor(swatch,color);div.appendChild(swatch);}
return div;}